...
class ZReader(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.imgThread = NetThread.ImageThread(self.google_api) #下载文中图片的线程
        #订阅列表上的右键菜单
        ...
        self.imgThread.finished.connect(self.refresh_context)
    ...
    def loadingSuccess(self, flag):
        #登入成功或切换显示视图的或由feed地址获取条目的线程成功后加载到tableWidget
        if flag == 'loading':
    ...
    def show_context(self):
        #点击一个条目后显示内容
        self.imgThread.exit()
        pattern = r"<\s*img.+?src=[\"\'](.+?(?:jpg|png|bmp|gif|jpeg))[\"\'].*?/>"
        com_obj = re.compile(pattern)
        matches = com_obj.finditer(self.current_item.content)
        link_list = []
        for match in matches:
            link_list.append(match.group(1))
        self.ui.textEdit.setHtml(self.current_item.content)
        self.imgThread.link_list = link_list
        self.imgThread.item = self.current_item
    ...
    def refresh_context(self):
        ...
        for index, link in enumerate(self.imgThread.link_list):
            content = content.replace(link, self.imgThread.img_list[index])
    ...
    def on_tableWidget_itemClicked(self, item):
        ...
        if column == 2:
            ...
            self.ui.tableWidget.item(row,column).setFont(font)
            self.current_item = self.current_list[row]
            self.show_context()
